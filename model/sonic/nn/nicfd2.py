...
for j in Me.index:
    ...
    """
    n4 = 1000
    p = np.linspace(P1[j]*4,P1[j]*(6*Me[j] - 7.5),n4) # must choose reasonable range
    p = pd.Series(p)
    h = np.zeros(p.size) 
    d = np.zeros(p.size) 
    u = np.zeros(p.size) 
...
pd.DataFrame(xx).to_csv('result2.csv', index_label = "Index", header  = ['X/De'])
