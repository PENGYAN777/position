...
for j in Me.index:
    ...
    """
    n4 = 1000
    p = np.linspace(P1[j]*16,P1[j]*20 ,n4) # must choose reasonable range
    p = pd.Series(p)
    h = np.zeros(p.size) 
    d = np.zeros(p.size) 
    u = np.zeros(p.size) 
...
pd.DataFrame(xx).to_csv('result4.csv', index_label = "Index", header  = ['X/De'])
data = pd.read_csv("result4.csv", ",")
D =pd.DataFrame({'Me': Me, 'M2': M2, 'P2/Pt': P2/Pt, 'T2/Tt': T2/Tt,'P1/Pt': P1/Pt,})
newData = pd.concat([data, D], join = 'outer', axis = 1)
newData.to_csv("result4.csv")
