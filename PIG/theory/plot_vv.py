...
p = pd.read_csv("paper_p.csv", ",", skiprows=0)
...
print("average diff: ", sum(abs(diff))/diff.size)

# fig 2
fig2 = plt.figure( dpi=300)
lwh = 2
axes = fig2.add_axes([0.15, 0.15, 0.7, 0.7]) #size of figure
axes.plot(sonic.iloc[:,2] , sonic.iloc[:,4], 'k+', lw=lwh, label="Theoretical solution")
axes.plot(p.iloc[:,0] , p.iloc[:,1], 'ko', lw=lwh, label="PhilipL 1948")
...
axes.set_ylim([0,1])
axes.set_xlabel('$X/D_e$',fontsize=12)
axes.set_ylabel('$P/P_e$',fontsize=12) 
# axes.set_title('$P/P_c$ along y$=0.3 mm$',fontsize=14)
axes.legend(loc=0) # 

fig2.savefig("sonic_vv_p.pdf")